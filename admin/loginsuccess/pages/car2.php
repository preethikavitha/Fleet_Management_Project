...
<div class="modal fade w-150" id="viewusermodal" tabindex="-1" role="dialog" aria-labelledby="viewusermodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="viewusermodalLabel"> Rented Vehicle Details</h5>
...
<script>
	...
	$sql="SELECT num_plate_id,brand,model,category,seat,transmission_type,status FROM vehicle WHERE status='Booked' && isActive='Active'";
...
<label for="drop">Entries to show</label>
							...
							if($result->num_rows>0)
							{
								while($row=$result->fetch_assoc())
								{
									...
									<td><?php echo "transmission type: ".$row['transmission_type']."<br>Seats: ".$row['seat']."<br>Status:<b>".$row['status']."</b>";?></td>
									<td class="text-center">
											  <button class="btn btn-sm btn-primary view_car"   data-id="<?php echo $row['num_plate_id'] ?>" type="button" >View</button>
											  
											  <button class="btn btn-sm btn-warning rent_car"   data-id="<?php echo $row['num_plate_id'] ?>" type="button" >Rent</button>
										  </td>
				...
				$.ajax(
					{
						...
						url:"livesearch.php",
						method:"POST",
						data:{'click_search_rent':true,input:input},
